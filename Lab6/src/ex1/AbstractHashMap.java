...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  private double maxLoadFactor;        // stores the maximum allowed load factor

  /** Creates a hash table with the given capacity and prime factor. */
  public AbstractHashMap(int cap, int p) {
    this(cap, p, 0.5);  // use 0.5 as the default load factor
  }
  ...
  public AbstractHashMap(int cap, int p, double loadFactor) {
    ...
    prime = p;
    capacity = cap;
    maxLoadFactor = loadFactor;   // save user's load factor

    Random rand = new Random();
    scale = rand.nextInt(prime-1) + 1;  // random MAD scale
    shift = rand.nextInt(prime);              // random MAD shift
    createTable();                            // create hash table
  }
  /**
  ...
  public V put(K key, V value) {
    ...
    if ((double) n / capacity > maxLoadFactor)  //  keep load factor within the chosen limit
      resize(2 * capacity - 1);        // grow the table
    return answer;
  }
