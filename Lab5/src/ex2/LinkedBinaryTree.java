...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
  ...
  // printSubtreeHeights method here
  public int printSubtreeHeights(Position<E> p) {
    // store the maximum height for children
    int maxChildHeight = -1;

    // visit children first (postorder)
    for (Position<E> c : children(p)) {
      int childHeight = printSubtreeHeights(c); // compute child subtree height
      if (childHeight > maxChildHeight) {
        maxChildHeight = childHeight;           // update max height
      }
    }

    // compute height of current node
    int height = maxChildHeight + 1;

    // print element and its subtree height
